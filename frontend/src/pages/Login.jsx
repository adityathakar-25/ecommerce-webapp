  ...
  return (
    <div className='min-h-[80vh] flex items-center justify-center'>
      <form onSubmit={onSubmitHandler} className='flex flex-col items-center w-[90%] sm:max-w-96 m-auto gap-4 text-gray-800'>
        ...
        {currentState === 'Login' ? '' : <input onChange={(e)=>setName(e.target.value)} value={name} type="text" className='w-full px-3 py-2 border border-gray-800 rounded' placeholder='Name' required/>}
        ...
        <button className='bg-black text-white font-light px-8 py-3 mt-4 rounded'>{currentState === 'Login' ? 'Sign In' : 'Sign Up'}</button>
      </form>
    </div>
  )
}
